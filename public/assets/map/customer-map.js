...
const PatientresultsWrapperHTML = document.getElementById("customer-search-result")

// initial map
const PatientMap = L.map("customer-render-map")
    ...
    fetch(`https://nominatim.openstreetmap.org/search?q=${keyword}&format=json`)
      .then((response) => {
       ...
       console.log("json", json)
        ...
        else document.getElementById('customer-location-notfound').text = 'Lokasi Tidak Di Temukan';
    })
  }
}
...
function PatientrenderResults(result) {
  ...
  result.map((n) => {
    resultsHTML += `<li class="address_list"><a href="#" id="customer_district_list${n.lat}" onclick="PatientsetLocation(${n.lat},${n.lon});">${n.display_name}</a></li>`
  })
...
function PatientsetLocation(lat, lon) {
  ...
  document.getElementById('customer_lon').value = lon;
  document.getElementById('customer_district').value = document.getElementById('customer_district_list'+lat).text;
